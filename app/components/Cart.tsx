  ...
  return (
    ...
    >
      <motion.div
        ...
        className=" bg-secondary shadow absolute right-0 top-0 w-[34%] h-screen  overflow-scroll flex flex-col items-center gap-5 bg-opacity-[0.98] "
      >
        <h1
          onClick={() => cartStore.toggleCart()}
        ...
        {cartStore.cart.length > 0 && (
          <motion.div
            layout
            className="flex flex-col gap-4 mt-10 w-[80%] mx-auto"
          >
            <p className="text-[1.2rem]">
              Total Price:{" "}
              <span className="gradientText">{formatPrice(totalPrice)}</span>
            </p>
            <button className="  py-[0.6rem]  text-[1.1rem] rounded-lg gradientBg">
              Checkout
            </button>
          </motion.div>
        )}
